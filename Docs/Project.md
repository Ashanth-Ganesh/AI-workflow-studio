...
`dev`, and CI/CD are architectural targets described below, not implemented
features. Where later sections use present tense for those targets, read it as
the intended design rather than a statement of current deployment status.
...
Ruff and Pytest are currently configured. A static type checker may be added
once the API grows beyond the authentication foundation.
...
**Hosted development database (planned):** Neon PostgreSQL

**Local database:** PostgreSQL Docker container

Neon is planned for the hosted `dev` environment because it provides
...
The only active environment is currently:

``` text
LOCAL
```

The planned next environment is:
...
`dev` is the intended first hosted environment. It has not been deployed yet.
...
## 6.1 Frontend Hosting (planned)

The React application is intended to be hosted using **Azure Static Web Apps**.
...
## 6.2 API Hosting (planned)

The FastAPI backend is packaged as a Docker image and is intended to be hosted using
...
## 6.3 Database Hosting (planned)

Hosted development should use **Neon PostgreSQL**, not Azure Database for
...
|-- apps/
|   |-- api/
|   |   |-- src/ai_workflow_studio/  # core, db, auth, and health modules
|   |   |-- alembic/                 # migration environment and revisions
|   |   |-- tests/
|   |   `-- Dockerfile
|   `-- web/                         # React + TypeScript + Vite UI
|-- Docs/Project.md                  # this architectural context
|-- .env.example
|-- docker-compose.yml
|-- pyproject.toml
|-- requirements.txt
...
Target monorepo structure as workflow capabilities are added:
...
Current variables and defaults are defined in `.env.example`; it is the source
of truth for local configuration. In addition to the categories above, the
implemented application uses `CORS_ORIGINS`, `CSRF_COOKIE_NAME`,
`OAUTH_COOKIE_NAME`, `SESSION_TTL_SECONDS`, and `COOKIE_SECURE`. The local
launcher sets the API container database host to the Compose service (`db`),
while direct local API runs use the `localhost` value in `.env`.
...
# 31. Target Architecture Summary (planned)
